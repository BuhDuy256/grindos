...
| `POST` | `/admin/daily-plan/with-tasks` | `{user_id, date, system_message, progress_analysis, tasks[]}` | `{plan_id}` |
| `PATCH` | `/admin/daily-plan/{id}` | `{ecr_score?, user_note?, learning_summary?, ai_insight?}` | `{ok: true}` |
